...
(function (w) {
  ...
  var canvas_width = Math.floor((w.screen.width*0.90)/resolution)*resolution; //Needs to be a multiple of the resolution value below.
  var canvas_height = Math.floor((w.screen.height*0.8)/resolution)*resolution; //This too.
  ...
  function update_particle() {
    ...
    for (i = 0; i < particles.length; i++) {
      ...
      if(Math.abs(p.xv) > 0.2){p.xv *= 0.5;}
      if(Math.abs(p.yv) > 0.2){p.yv *= 0.5;}

      // if(Math.abs(p.xv) < 0.1){
      //   if(p.xv >= 0){
      //     p.xv = (Math.random() + 0.1)
      //   }else{
      //     p.xv = -1*Math.random()-0.1
      //   }
      // }
      // if(Math.abs(p.yv) < 0.1){
      //   if(p.yv >= 0){
      //     p.yv = (Math.random() + 0.1)
      //   }else{
      //     p.yv = -1*Math.random()-0.1
      //   }
      // }
  ...
  function touch_end_handler(e) {
    if (!e.touches) mouse.down = true; //If there are no more touches on the screen, sets "down" to false.
  }
...
/*
Request animatioyn frame polyfill. This enables ou to use "requestAnimationFrame" 
